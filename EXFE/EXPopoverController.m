...
- (void)presentFromRect:(CGRect)rect inView:(UIView *)view arrowDirection:(EXArrowDirection)direction animated:(BOOL)animated complete:(void (^)(void))handler {
    ...
    if (direction & kEXArrowDirectionDown) {
        ...
        if (x >= 0) {
            finalArrowDirection = kEXArrowDirectionRight;
        ...
        CGFloat x = arrowPoint.x - maxViewWidth;
        if (x >= 0) {
            finalArrowDirection = kEXArrowDirectionLeft;
